# Tier C sequence — broad / lower-fit

Smaller, less-established, or lower-fit businesses. Keep it short and low-effort —
this is a volume tier. Still human, never spammy.

Merge fields: `{{FirstName}}` `{{Business}}` `{{CalLink}}` `{{YourAddress}}`.

---

## Email 1 — intro

**Subject:** a quick automation idea

Hi {{FirstName}},

I'm Chauncey — I build AI automations for local businesses here in West LA: the repetitive stuff like scheduling, intake, follow-ups, and reporting that eats your team's week.

If you've got a task that feels like it should run itself, it's usually one I can automate. Happy to do a free 15-minute call and take a look — no pressure either way.

{{CalLink}}, or just reply.

— Chauncey Tse · previously at Google, Amex & KPMG
chauncey.tse@gmail.com · tsecapital.co

<sub>{{YourAddress}} · Not useful? Reply "no thanks" and I won't write again.</sub>

---

## Email 2 — follow-up (optional, send ~5 business days later, same thread)

Hi {{FirstName}},

Quick follow-up — still glad to take a look at where automation could save {{Business}} a few hours a week. Free, no obligation: {{CalLink}}.

If it's not for you, no worries at all.

— Chauncey
